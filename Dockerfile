...
ARG USER_NAME="eric-oss-asn1-decoder-poc"
RUN echo "$USER_ID:x:$USER_ID:0:An Identity for $USER_NAME:/nonexistent:/bin/false" >>/etc/passwd

ARG APP_MODULE=eric-oss-asn1-decoder-poc
ARG APP_VERSION
ADD target/${APP_MODULE}-${APP_VERSION}.jar ${APP_MODULE}.jar
...
CMD ["/bin/sh", "-c", "java ${JAVA_OPTS} -Dcom.sun.management.jmxremote=true \
    -Dcom.sun.management.jmxremote.port=${JMX_PORT} -Dcom.sun.management.jmxremote.authenticate=true \
    -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.rmi.port=${JMX_PORT} \
...
LABEL \
    org.opencontainers.image.title=eric-oss-asn1-decoder-poc-jsb \
